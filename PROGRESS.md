...
> **最終更新**: 2026-03-22
> **次の作業再開時は `tasks/todo.md` を参照すること（事業横断タスクの正本）**
> このファイルは事業概要・インフラ構成の記録。タスク管理は `tasks/todo.md` で行う。
...
> **詳細・全プロジェクトのインフラ情報**: [`docs/infra/`](./docs/infra/README.md) を参照。
> このセクションはコーポレートサイト（eleanor-dev.com）のクイックリファレンス。
